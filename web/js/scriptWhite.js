    ...
    fetch(url)
        .then(async res => {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                if (pieceName.includes("pieceBlack")) {
                    movePiece(piecesBlackSet[i][0], [data[i].position.posX, data[i].position.posY])
                }
            }
            // movePiece(piecesBlackSet[0][0], [4,4])

            // for(i=12;i<24;i++){
            //     let lastX = data[i].posX[data[i].posX.length - 1]
            //     let lastY = data[i].posY[data[i].posY.length - 1]
            //     movePiece(piecesBlackSet[i-12][0],[lastX,lastY])
            // }
        })
}
setInterval(() => {
...
document.addEventListener("pointerdown", () => {
    ...
    console.log("start", piecesWhiteSet[0][0].position.x, ",", piecesWhiteSet[0][0].position.z)
    var setIntersects = [];
    for (let i = 0; i < 12; i++) {
        setIntersects.push(piecesWhiteSet[i][0])
    }
...
document.addEventListener("pointerup", () => {
    ...
    dragObject = null;
    for (let i = 0; i < 12; i++) {
        piecesWhiteSet[i][0] = fixPosition(piecesWhiteSet[i][0]);
        // postData(piecesWhiteSet[i][0],`pieceWhite${i}`);
        patchData(piecesWhiteSet[i][0], `pieceWhite${i}`)
        console.log("finish ", i, " ", piecesWhiteSet[i][0].position.x, ",", piecesWhiteSet[i][0].position.z);
    }
